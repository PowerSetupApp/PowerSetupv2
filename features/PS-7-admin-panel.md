...
- **Lücken-Filter** (mehrere Checkboxen, **ODER**): (1) **Unvollständige Filter-Werte** — `CategoryFilter`-Keys der Kategorie (außer **`brand`**) fehlen/leer in `filterValues`; (2) **Ohne Foto oder Preis**; (3) **Ohne Algorithmus-Spec** — wie Katalogabdeckung (`powerW` / `currentA` / `crossSectionMm2` je Kategorie-Slug).
