...
export default function ResultJsonViewer({
    ...
    const formattedText = useMemo(() => {
        ...
        try {
            // Enhance formData with calculated values from calculations
            const enhancedFormData = {
                ...(formData as object),
                calculatedDailyWh: (calculations as any)?.battery?.dailyWh ?? null,
            };
            const formDataStr = formatFormDataForAI(enhancedFormData as unknown as Parameters<typeof formatFormDataForAI>[0]);
    ...
    const compactText = useMemo(() => {
        ...
        try {
            ...
            return formatFormDataCompact(enhancedFormData as unknown as Parameters<typeof formatFormDataCompact>[0]);
        } catch (e) {
            return 'Fehler';
        }
    }, [formData, calculations]);
