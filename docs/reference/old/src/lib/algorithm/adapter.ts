...
import { calculateRequirements, getPSH } from './algorithm';
...
export function convertToSystemRequirements(
    ...
    return {
        ...
        battery: {
            ...
            maxCapacityAh: Math.ceil(output.battery.recommendedCapacityAh * 1.5 / 10) * 10, // 50% headroom, rounded to 10Ah
        ...
        booster: output.booster.needed ? {
            ...
            alternatorType: 'standard', // Default – Euro 6d smart alternators may need different booster type
        } : null,
        ...
        solarController: output.controller.needed ? {
            ...
            recommendedCurrentA: output.controller.currentA,
            type: output.controller.type.toUpperCase() as 'MPPT' | 'PWM',
            needsSeparatePortableController: output.solar.portableWp > 0 && output.solar.maxRoofWp > 0,
        } : null,

        solarModules: output.solar.needed ? {
            needed: output.solar.needed,
            dailyWh: output.solar.dailySolarYieldWh,
            sunHoursPerDay: getPSH(input.travelBehavior.season, input.travelBehavior.winterLocation, input),
 ...
 */
export function convertWizardInputToAlgorithmInput(
    wizard: WizardInput,
    ...
    // Map autarchy days to nearest valid step (rounds up to avoid under-dimensioning)
    ...
    // Note: values between steps (e.g. days=3) round up to the next step (6).
    // This is intentional – the algorithm's discrete steps prevent under-dimensioning.
 ...
 */
export async function calculateSystemRequirements(
    input: WizardInput,
    settings?: Partial<AlgorithmSettingsData>
): Promise<SystemRequirements> {
    // Convert wizard input to new algorithm format
    const algorithmInput = convertWizardInputToAlgorithmInput(input, settings);
...
export function preFilterProducts(
    ...
    return products.filter(p => {
        ...
        // Only enforce pure_sine when 230V loads are present (modified_sine is fine for simple loads)
        if (slug === 'wechselrichter') {
            const has230VLoad = (requirements.inverter?.total230VLoadW ?? 0) > 0;
            if (has230VLoad && p.waveform && p.waveform !== 'pure_sine') return false;
