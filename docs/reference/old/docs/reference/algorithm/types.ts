...
export interface AlgorithmInput {
     ...
     * If provided, these values override the hardcoded constants
     */
    settings?: Partial<AlgorithmSettingsData>;
}

/**
 * Settings data structure (matching Prisma model)
 */
export interface AlgorithmSettingsData {
    ...
    solarControllerSafetyFactor?: number;

    // Booster
    defaultBoosterAmps?: number;
    boosterEfficiency?: number;
    alternatorDriveHours?: number;

    // Charger
    chargerTimeSlow?: number;
    chargerTimeNormal?: number;
    chargerTimeFast?: number;

    // Inverter / Loads
    simultaneousFactorLow?: number;
    simultaneousFactorModerate?: number;
    simultaneousFactorHigh?: number;
