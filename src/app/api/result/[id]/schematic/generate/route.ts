...
export async function POST(
    ...
    try {
        ...
        console.log("Generating Schematic with Prompt Length:", prompt.length);
        ...
        if (process.env.BLOB_READ_WRITE_TOKEN) {
            try {
                ...
                // Fallback to OpenAI URL (expires!)
            }
        }
        ...
        await prisma.result.update({
            where: { id },
            data: { pdfUrl: finalUrl } // Using pdfUrl field for the schematic image for now
        });
