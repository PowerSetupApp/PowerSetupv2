...
function calculateMatchScore(
    ...
    if (requirement.minValue !== undefined && typeof mainValue === 'number') {
        if (mainValue >= requirement.minValue) {
            score += 50;
    ...
    if (requirement.additionalFilters) {
        for (const [filterKey, expectedValue] of Object.entries(requirement.additionalFilters)) {
            ...
            if (filterKey.toLowerCase().includes('voltage')) {
    ...
    // 4. Marken-Präferenz (+20 Punkte)
    if (brandPreferences && product.category?.slug) {
        const categoryBrandPref = getBrandPreferenceForCategory(product.category.slug, brandPreferences);
        if (categoryBrandPref) {
            // Hier müsste man product.brandId mit der Präferenz vergleichen
            // Vereinfacht: Wenn Brand-Name im Produktnamen vorkommt
            // In der Realität sollte man product.brandId mit dem Brand-Model abgleichen
            score += 10; // Teilpunkte wenn überhaupt eine Marke verfügbar
        }
    }
...
function getProductValue(
    ...
    const fieldMapping: Record<string, keyof ProductWithFilter> = {
        ...
        'inputVoltage': 'supportedVoltages',
        'outputVoltage': 'supportedVoltages',
    ...
    for (const req of categoryReqs) {
        ...
        if (req.categorySlug === 'batterien') {
            candidates.sort((a, b) => {
                ...
                // 2. Priorität: Wenn beide erfüllen, nimm die mit Score
                if (Math.abs(a.matchScore - b.matchScore) > 10) {
                    return b.matchScore - a.matchScore;
                }

                // 3. Priorität: Wenn beide erfüllen (und ähnlicher Score), nimm die GRÖSSERE Kapazität (weniger Overhead)
                // Aber nicht unnötig riesig -> Nimm die, die näher am Target ist, aber drüber
                const capA = getProductValue(a.product, {}, 'capacityAh') as number || 0;
                const capB = getProductValue(b.product, {}, 'capacityAh') as number || 0;
                const target = req.targetValue || 100;

                // Beide sind >= target (da meetsMinimum true), also nimm die, die näher dran ist (kleinerer Überschuss)
                // ODER der User will "Single Large", dann ist größer vielleicht besser?
                // Wir nehmen: "Besser eine 200er als 2x100er" -> das ist hier schon durch meetsMinimum abgedeckt.
                // Vergleich: 200Ah (1x) vs 280Ah (1x) für 150Ah Bedarf.
                // Beide meetsMinimum.
                // Wir wollen die passendere (näher am Target).

                return a.matchScore - b.matchScore; // Fallback auf Score
            });
        }
