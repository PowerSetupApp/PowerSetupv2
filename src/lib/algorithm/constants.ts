...
/**
 * Autarchy days at or below this threshold get **no** shore-power soft-bridge
 * relief — short off-grid windows are sized the same with or without
 * `shore_power` selected (product rule).
 */
export const SHORE_BATTERY_RELIEF_AUTARCHY_THRESHOLD_DAYS = 2;

/**
 * Soft-bridge day credit [days] from shore **after** the autarchy streak —
 * subtracted from `min(autarchyDays, AUTARCHY_MAX_BRIDGE_DAYS)` before
 * multiplying by `bridgeDailyDeficitWh`. Keyed on `shoreAvailability`
 * (`derive.shoreAvailability`). `never` must be `0`.
 *
 * Values are capped implicitly by `AUTARCHY_MAX_BRIDGE_DAYS` (7) in practice;
 * `full_time` is the strongest relief.
 */
export const SHORE_BRIDGE_RELIEF_DAYS: Record<ShoreAvailability, number> = {
  never: 0,
  occasional: 0.75,
  nightly: 1,
  nightly_fast: 1.25,
